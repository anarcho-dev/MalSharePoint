...
class User(db.Model):
    ...
    must_change_password = db.Column(
        db.Boolean, server_default="0", default=False, nullable=False
    )
    created_at = db.Column(db.DateTime(timezone=True), default=_utcnow, nullable=False)
    last_login = db.Column(db.DateTime(timezone=True))

    uploads = db.relationship(
        "File", backref="uploader", lazy="dynamic", foreign_keys="File.uploaded_by"
    )
    audit_logs = db.relationship(
        "AuditLog", backref="user", lazy="dynamic", foreign_keys="AuditLog.user_id"
    )
...
class ListenerProfile(db.Model):
    ...
    default_response_body = db.Column(
        db.Text, default="<html><body><h1>It works!</h1></body></html>"
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "custom_headers": (
                json.loads(self.custom_headers) if self.custom_headers else {}
            ),
...
class Listener(db.Model):
    ...
    options = db.Column(
        db.Text, default="{}"
    )  # JSON based extra configuration for different protocols

    callbacks = db.relationship("Callback", backref="listener", lazy="dynamic")
    staged_payloads = db.relationship(
        "StagedPayload", backref="listener", lazy="dynamic"
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "last_stopped_at": (
                self.last_stopped_at.isoformat() if self.last_stopped_at else None
            ),
            "created_at": self.created_at.isoformat(),
...
class StagedPayload(db.Model):
    ...
    listener_id = db.Column(db.Integer, db.ForeignKey("listeners.id"), nullable=False)
    payload_type = db.Column(
        db.String(32), nullable=False, default="raw"
    )  # raw|ps1|bat|vbs|hta
    content = db.Column(db.Text, nullable=False)
...
class Agent(db.Model):
    ...
    )  # active|dormant|dead|disconnected
