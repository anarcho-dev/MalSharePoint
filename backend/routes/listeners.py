...
def create_listener():
    ...
    if listener_type == "https":
        if not tls_cert_path or not tls_key_path:
            return (
                jsonify({"error": "HTTPS requires tls_cert_path and tls_key_path"}),
                400,
            )
...
def update_listener(lid):
    ...
    if "options" in data:
        listener.options = json.dumps(data["options"])
...
def list_staged(lid):
    payloads = (
        StagedPayload.query.filter_by(listener_id=lid)
        .order_by(StagedPayload.created_at.desc())
        .all()
    )
...
def get_staged_delivery(lid, sid):
    ...
    return jsonify({"raw_url": raw_url, "commands": commands}), 200
...
def update_staged(lid, sid):
    ...
    if "stage_path" in data:
        ...
        if new_path.startswith("/"):
            existing = StagedPayload.query.filter_by(
                listener_id=lid, stage_path=new_path
            ).first()
...
def create_staged_from_template(lid):
    ...
    if "LHOST" not in params:
        params["LHOST"] = (
            listener.bind_address if listener.bind_address != "0.0.0.0" else "127.0.0.1"
        )
